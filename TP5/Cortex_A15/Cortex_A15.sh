...
for NTHREADS in "${THREADS_LIST[@]}"; do
    for WIDTH in 2 4 8; do
        OUTPUT_DIR="m5out_m${MATRIX_SIZE}_t${NTHREADS}_w${WIDTH}"
        ...
        $GEM5_EXEC $CONFIG_FILE -n $NTHREADS -c $EXECUTABLE -o "$NTHREADS $MATRIX_SIZE" --cpu-type=arm_detailed --caches -w $WIDTH
        ...
        mv m5out "$OUTPUT_DIR"
        echo "Simulation terminée pour NTHREADS=$NTHREADS, WIDTH=$WIDTH résultats enregistrés dans $OUTPUT_DIR"
    done
done
